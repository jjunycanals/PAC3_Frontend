...
export class ProfileComponent implements OnInit {
  ...
  constructor(
    ...
    this.profileForm = this.formBuilder.group({
      ...
      birth_date: [formatDate(this.profileUser.birth_date, 'yyyy-MM-dd', 'en'), [Validators.required]],
    ...
    this.store.select('user').subscribe((user) => {
      ...
      this.profileForm.patchValue({
        ...
        birth_date: formatDate(this.profileUser.birth_date, 'yyyy-MM-dd', 'en'),
  ...
  updateUser(): void {
    ...
    this.isValidForm = true;
    const updatedUser: UserDTO = this.profileForm.value;

    if (this.userId) {
      this.store.dispatch(UserAction.updateUser({ userId: this.userId, user: updatedUser }));
    }
  }
}
